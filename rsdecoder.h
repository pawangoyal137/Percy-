...
/// @tparam FX	    Univariate polynomials over the field F.
template <class FX>
struct RecoveryPoly {
    /// Constructor.
    /// @param G    Server indices that agree.
    /// @param phi  Reconstructed polynomial.
...
struct RecoveryPolyMulti {
    /// Constructor.
    /// @param G    Server indices that agree.
    /// @param phis Reconstructed polynomials.
...
/// @tparam F	The field used for decoding.
template <class F>
struct DecoderResult {
    /// Constructor.
    /// @param G	    Server indices that agree.
    /// @param recovered    Recovered words by word index
    DecoderResult (vector<nservers_t> G, vector<map<dbsize_t, F> > recovered) : 
	    G(G), recovered(recovered) {}
...
/// fields.  
// It's been tested on ZZ_p and GF2E.  I haven't tried it over
...
/// @tparam F	    The field used for decoding.
/// @tparam vec_F   Vectors over the field F.
/// @tparam FX	    Univariate polynomials over the field F.
/// @tparam FXY	    Bivariate polynomials over the field F.
/// @tparam mat_F   Matrices over the field F.
...
class RSDecoder {
	...
	/// @param bytes_per_word	The number of bytes each word uses.
	/// @param t			The privacy level.  I.e. the maximum
	///				number of servers that can collude 
	///				without breaking privacy.
	/// @param h			The number of honest servers.
	/// @param goodservers		The indices of the servers that did not
	///				act up.
	/// @param values		The server shares.  Indexed by query
	///				then by word.
	/// @param server_indices	The indices used for server shares.
	/// @param results		The decoding results for each query.
	///				Will be filled as decoding occurs.
	/// @param decoded		For each query, the words that have been
	///				successfully decoded.  Will be updated
	///				as decoding occurs.
	/// @param vec_interp_indices	The indices corresponding to each block
	///				requested.
	/// @param multi_only		The number of queries that should only
	///				be processed using multi-query decoding.
	///				The first multi_only queries were
	///				previously attempted, were not 
	///				successfully decoded and were saved for
	///				multi-query decoding.
	/// @return			Return true if all queries were decoded,
	///				false otherwise.
	bool Recover (dbsize_t bytes_per_word, nservers_t t, nservers_t h,
		...
		const vec_F &server_indices, vector<vector<DecoderResult<F> > > &results,
	...
	/// @param wz		    The fields element to append to the string.
	/// @param bytes_per_word   The number of bytes each word uses.
	/// @return		    The string s appended with wz.
	static string append(const string &s, const F &wz,
		unsigned int bytes_per_word);

	/// Interpolate some points I and check how many points G agree with the
	/// result.
	...
	/// @param t		The degree of the interpolating polynomial.
	/// @param values	The values of the points.
	/// @param indices	The indices of the points.
	/// @param I		The indices of the points to interpolate.
	/// @param G		The indices of the points to check with phi.
	/// @param numagree	Number of points in G that agree with phi.
	/// @param numdisagree	Number of points in G that do not agree with 
	///			phi.
	/// @param vecagree	The indices of the points in G that agree with 
	///			phi.
	/// @param phi		The interpolant.
